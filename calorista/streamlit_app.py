...
def get_redis_connection():
    ...
    try:
        ...
        st.error(
            f"Could not connect to Redis. Please check your .env file and Redis server. Error: {e}"
        )
...
if not food_df.empty:
    ...
    st.write(
        f"Data available from **{min_available_date.strftime('%Y-%m-%d')}** to **{max_available_date.strftime('%Y-%m-%d')}**"
    )
    ...
    if latest_date:  # Only display if a latest_date exists
        ...
        if not latest_day_df.empty:
            ...
            st.info(
                f"No entries for the latest day: {latest_date.strftime('%Y-%m-%d')}."
            )
    else:
        st.warning(
            "No date entries found in your Redis database that match the 'food_entries:YYYY-MM-DD' key pattern for the latest day."
        )
    ...
    default_start_date_picker = max(
        min_available_date, max_available_date - timedelta(days=6)
    )
    ...
    if selected_start_date > selected_end_date:
        st.error(
            "Error: Start date cannot be after end date. Please adjust your selection."
        )
    else:
        st.write(
            f"Displaying data from **{selected_start_date.strftime('%Y-%m-%d')}** to **{selected_end_date.strftime('%Y-%m-%d')}**"
        )

        filtered_df = food_df[
            (food_df["date"] >= selected_start_date)
            & (food_df["date"] <= selected_end_date)
        ].copy()
        ...
        if not filtered_df.empty:
            ...
            date_range = pd.date_range(
                start=selected_start_date, end=selected_end_date, freq="D"
            ).date
            daily_totals_filtered = (
                daily_totals_filtered.set_index("date")
            ...
            st.info(
                f"No entries found for the selected date range from {selected_start_date.strftime('%Y-%m-%d')} to {selected_end_date.strftime('%Y-%m-%d')}."
            )
...
if not food_df.empty:
    ...
    if not weekly_totals.empty:
        ...
        weekly_totals[numeric_cols] = weekly_totals[numeric_cols].apply(
            pd.to_numeric, errors="coerce"
        )

        # Calculate daily averages
        weekly_totals["avg_daily_calories"] = (
            weekly_totals["total_calories"] / weekly_totals["days_logged"]
        )
        weekly_totals["avg_daily_carbs"] = (
            weekly_totals["total_carbohydrate"] / weekly_totals["days_logged"]
        )
        weekly_totals["avg_daily_fat"] = (
            weekly_totals["total_fat"] / weekly_totals["days_logged"]
        )
        weekly_totals["avg_daily_protein"] = (
            weekly_totals["total_protein"] / weekly_totals["days_logged"]
        )
        ...
        st.dataframe(
            weekly_totals[display_cols].rename(
                columns={
                    "week_label": "Week",
...
if not food_df.empty:
    ...
    monthly_totals = (
        food_df.groupby(["month_start", "month_label"])
        .agg(
            total_calories=("calories", "sum"),
            total_carbohydrate=("carbohydrate", "sum"),
            total_fat=("fat", "sum"),
    ...
    if not monthly_totals.empty:
        # Calculate daily averages
        monthly_totals["avg_daily_calories"] = (
        ...
        monthly_totals["avg_daily_protein"] = (
            monthly_totals["total_protein"] / monthly_totals["days_logged"]
        )
        ...
        st.dataframe(
            monthly_totals[display_cols].rename(
                columns={
                    "month_label": "Month",
                    "total_calories": "Total Calories",
                    "avg_daily_calories": "Avg Daily Calories",
